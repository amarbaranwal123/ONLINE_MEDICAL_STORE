...
$database = new CreateDb("Productdb", "elder");
...
if(isset($_POST['add'])){
	...
	if(isset($_SESSION['cart'])){
		...
		if(in_array($_POST['product_id'], $item_array_id)){
			...
			echo "<script>window.location = 'elder.php'</script>";
		}else{
	...
	<div class="row text-center py-5">
		<?php
			...
			while($row = mysqli_fetch_assoc($result)){
				component13($row['product_name'], $row['product_price'], $row['product_image'], $row['id']);
			}
		?>
	</div>
