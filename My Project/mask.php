...
$database = new CreateDb("Productdb", "mask");
...
if(isset($_POST['add'])){
	...
	if(isset($_SESSION['cart'])){
		...
		if(in_array($_POST['product_id'], $item_array_id)){
			...
			echo "<script>window.location = 'mask.php'</script>";
		}else{
...
<div class="feedback">
		<div class="cbb">
		<center><h2>ANTI-POLLUTION MASKS</h2></center>
	</div>
</div>
	...
	<div class="row text-center py-5">
		<?php
			...
			while($row = mysqli_fetch_assoc($result)){
				component10($row['product_name'], $row['product_price'], $row['product_image'], $row['id']);
			}
		?>
	</div>
