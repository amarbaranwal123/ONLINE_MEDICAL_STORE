...
$database = new CreateDb("Productdb", "protein");
...
if(isset($_POST['add'])){
	...
	if(isset($_SESSION['cart'])){
		...
		if(in_array($_POST['product_id'], $item_array_id)){
			...
			echo "<script>window.location = 'protein.php'</script>";
		}else{
...
<div class="feedback">
		<div class="cbb">
		<center><h2>PROTEIN SUPPLEMENTS</h2></center>
	</div>
</div>
	...
	<div class="row text-center py-5">
		<?php
			...
			while($row = mysqli_fetch_assoc($result)){
				component6($row['product_name'], $row['product_price'], $row['product_image'], $row['id']);
			}
		?>
	</div>
